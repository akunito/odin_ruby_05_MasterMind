...
module MasterMind
  ...
  class ComputerDecoder < Player
    ...
    def calculate_decodes!
      ...
      # LO QUE BORRO DE writable_indexes_session SE BORRA DE writable_indexes ??????
      readable_indexes.each do |read_index|
        log_debug "\n"
        # reading value
        log_debug "reading value: #{@game.decoder_board[@game.round-1][read_index]}"
        ...
        writable_indexes_session.each_with_index do |e, i|
          if writable_indexes_session.include?(read_index)
            log_debug "writable_indexes_session contains: #{writable_indexes_session}"
            log_debug "removing #{read_index} from writable_indexes_session"
        ...
        log_debug "read_index: #{read_index}"
        log_debug "write_index: #{write_index}"
        # write decoder board
      ...
      log_debug "------------ readable_indexes after 'White' #{readable_indexes}"
      log_debug "------------ writable_indexes after 'White' #{writable_indexes}"
